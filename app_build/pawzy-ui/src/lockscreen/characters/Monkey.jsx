...
const VIDEO_ENTRY = './characters/monkey_entry_sbs.webm?v=4';
const VIDEO_IDLE  = './characters/monkey_idle_sbs.webm?v=4';
...
export function Monkey({ phase, onSlideEnd }) {
  ...
  useEffect(() => {
    ...
    video.play().catch(err => console.error('Monkey play error:', err));
  ...
  return (
    <div style={phase === 'slide' ? s.slideWrap : s.idleWrap}>
      <video
        ...
        onError={e => console.error('Monkey video error:', e.target.error)}
        style={{ display: 'none' }}
      />
